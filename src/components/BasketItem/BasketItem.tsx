...
const BasketItem: FC<BasketItemProps> = ({product, count}) => {
    const {deleteFromCart, increaseItemCount, decreaseItemCount} = useBasketStore()
            ...
            <div className={styles.basketItem__info}>
                ...
                <div className={styles.basketItem__countPrice}>
                    <div className={styles.basketItem__buttons}>
                        <button onClick={() => decreaseItemCount(product.id)}><DecreaseIcon/></button>
                        <p>{count}</p>
                        <button onClick={() => increaseItemCount(product.id)}><IncreaseIcon/></button>
                    </div>
                ...
                <div className={styles.basketItem__removeBtn}>
                    <button onClick={() => deleteFromCart(product.id)}><RemoveIcon /></button>
                </div>
            </div>
        </div>
    );
